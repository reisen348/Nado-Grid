...
5. 使用 `观察辅助` 查看当前价格在区间内的位置、下一入场、配对止盈、止盈止损距离。
6. 使用 `市场价`、`下沿`、`中线`、`上沿` 快速刷新不同价格场景的预览。
7. 执行模式保持 `模拟`，点击 `启动` 只做 dry-run 下单和审计记录。
8. 结合 TradingView 图表确认价格行为后，再手动决定是否调整区间或停止策略。
